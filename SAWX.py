...
def menu(my_name,my_id):
	...
	cetak(nel(oi, title='SAWx404'))
	jh = input(x+'telegram '+p+'instagram jb1lr '+x+' SAW #--> : ')
	if jh in ['1','01']:
		dump_publik()
	elif jh in ['2','02']:
		dump_massal()
...
def setting():
	...
	cetak(nel(tak, title='SAW'))
	hu = input(h+''+h+'SAW'+h+' 404: ')
	if hu in ['1','01']:
		...
		exit()
	met = '# SAWx4 ().print(mark(met, style='yellow'))
	ioz = '[bold green][01] M-FACEBOOK\n[02] FREE-FACEBOOK\n[03] MBASIC-FACEBOOK[/bold green]'
	gess = nel(ioz, style='ffffffcd')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%sMJ-BRUTE %s/%s OK:%s CP:%s  %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
